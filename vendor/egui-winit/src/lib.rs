        ...
        if let Some(active_key) = logical_key.or(physical_key) {
            if pressed {
                if is_cut_command(self.egui_input.modifiers, active_key) {
                    ...
                    // No text on the clipboard (often image-only). Still emit Key so the
                    // app can implement Ctrl/Cmd+V image paste — stock egui-winit would
                    // swallow V and the app would never see the shortcut.
