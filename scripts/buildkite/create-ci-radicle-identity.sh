...
install rad, materialize \$RAD_HOME, start the node, hydrate the RID into
local storage from Garden HTTPS (fallback: public radicle seeds), then
\`rad init --existing\` so \`git push rad HEAD:refs/patches\` works.
