...
install_cursor_agent() {
  ...
  echo "[bootstrap] Cursor CLI: $(bk_cursor_agent_bin)"
}
...
# Parent shells (run-issue-agent) invoke bootstrap as a subprocess — persist the
# resolved binary path so delegate does not depend on subprocess PATH.
if ! bk_persist_cursor_agent_bin; then
  bk_die "cursor-agent / agent not found after bootstrap"
fi

echo "[bootstrap] ready (repo=$REPO_ROOT rad_home=$RAD_HOME cursor_agent=$CURSOR_AGENT_BIN)"
