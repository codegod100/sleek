...
# artifacts.latha.org until the rebuilt artifact shows up (or timeout).
#
...
marker="ci: artifacts.latha.org pipeline smoke test $(date -u +%FT%TZ)"
git commit --allow-empty -m "$marker" >/dev/null
sha="$(git rev-parse HEAD)"
echo "pushing $sha to tangled/main ..." >&2
git push tangled main

url="https://artifacts.latha.org/artifacts/$sha/sleek.apk"
